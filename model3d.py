...
def render_mesh(frame, mesh, yaw=0.0, pitch=0.0, roll=0.0, scale=1.0,
    ...
    tri = screen[mesh.faces]            # (M, 3, 2)

    # --- Bước 5: cull mặt sau bằng dấu của diện tích tam giác đã chiếu ---
    ax = tri[:, 1, 0] - tri[:, 0, 0]
    ay = tri[:, 1, 1] - tri[:, 0, 1]
    bx = tri[:, 2, 0] - tri[:, 0, 0]
    by = tri[:, 2, 1] - tri[:, 0, 1]
    area = ax * by - ay * bx
    visible = area < 0 if not wireframe else np.ones(len(tri), dtype=bool)

    # Bỏ luôn các mặt nằm hoàn toàn ngoài khung hình
    tri_min = tri.min(axis=1)
    tri_max = tri.max(axis=1)
    on_screen = ((tri_max[:, 0] >= 0) & (tri_min[:, 0] < width) &
                 (tri_max[:, 1] >= 0) & (tri_min[:, 1] < height))
    visible &= on_screen
    if not visible.any():
        return 0

    face_idx = np.nonzero(visible)[0]
    faces = mesh.faces[face_idx]
    tri = tri[face_idx]

    # --- Bước 6: sắp xếp theo độ sâu, mặt xa vẽ trước ---
    depth = z[faces].mean(axis=1)
    order = np.argsort(-depth)
    faces = faces[order]
    tri = tri[order]

    # --- Bước 7: độ sáng từng mặt theo định luật Lambert ---
    p0 = points[faces[:, 0]]
    p1 = points[faces[:, 1]]
    p2 = points[faces[:, 2]]
    ...
    shade = 0.28 + 0.72 * lambert        # 0.28 là ánh sáng môi trường (ambient)

    base = np.array(base_color, dtype=np.float32)
    colors = np.clip(shade[:, None] * base, 0, 255).astype(np.int32)

    target = frame if alpha >= 0.999 else frame.copy()
    tri_int = np.round(tri).astype(np.int32)

    if wireframe:
        cv2.polylines(target, tri_int, isClosed=True, color=tuple(int(c) for c in base),
                      thickness=1, lineType=cv2.LINE_AA)
    else:
        for polygon, color in zip(tri_int, colors):
            cv2.fillConvexPoly(target, polygon, (int(color[0]), int(color[1]), int(color[2])),
